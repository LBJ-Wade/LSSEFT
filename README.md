...
The current release of *LSSEFT* is 2018.1.
...
- 2018.1 (9 May 2018) Source code [](https://doi.org/10.5281/zenodo.XXXXXX)
...
From version 2018.1 onwards, *LSSEFT* does not contain the code for any loop
integrals itself. Instead, it expects this code to be _autogenerated_
using the *LSSEFT-analytic* tool, which is available from
its own [repository](https://github.com/ds283/LSSEFT-analytic).
Download and install it, then run with whatever clustering and bias model
you wish. The one-loop integrals should be written into the *LSSEFT* source directory
in a folder labelled `autogenerated`.

In addition to *LSSEFT-analytic*, *LSSEFT* depends on a number of other libraries:
