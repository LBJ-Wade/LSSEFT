...
namespace sqlite3_operations
  {
    ...
    lookup_filter_params(sqlite3* db, transaction_manager& mgr, const Pk_filter_params& data,
                         ...
                         double tol)
      {
        ...
        select_stmt
          << "SELECT id FROM " << tokenization_table<filter_params_token>(policy) << " WHERE "
        ...
        while((status = sqlite3_step(stmt)) != SQLITE_DONE)
          {
            if(status == SQLITE_ROW)
              {
                if(id) throw runtime_exception(exception_type::database_error, ERROR_SQLITE3_MULTIPLE_FILTER_PARAMS);
    ...
    unsigned int
    insert_filter_params(sqlite3* db, transaction_manager& mgr, const Pk_filter_params& data,
                         const sqlite3_policy& policy)
      {
        assert(db != nullptr);
        
        // get number of rows in table; this will be the identifier for the new data set
        unsigned int new_id = count(db, tokenization_table<filter_params_token>(policy));
        
        std::ostringstream insert_stmt;
        insert_stmt
          << "INSERT INTO " << tokenization_table<filter_params_token>(policy) << " VALUES (@id, @amplitude, @pivot, @index, @abserr, @relerr);";
        ...
        // perform insertion
        check_stmt(db, sqlite3_step(stmt), ERROR_SQLITE3_INSERT_FILTER_PARAMS_FAIL, SQLITE_DONE);
